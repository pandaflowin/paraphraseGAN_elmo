...
def prepare_discriminator_data(oracle, gen, batch_size, is_val=False, on_cpu=True, gpu=False, gpu_limit=None):
    ...
    _, cond_seq_len, _ = cond_samples.shape
...
def pad_samples(samples, pad_token):
    ...
    if type(samples) != torch.Tensor:    
        ...
        lens = np.array([sum(1 if sum(ids) else 0 for ids in x) for x in samples.numpy()])
        lens = torch.LongTensor(lens)
...
def cat_samples(samples_1, lens_1, samples_2, lens_2, pad_token):
    ...
    # Concat & pad
    if len(samples_1.shape) == 3:
        samples = np.ones((num_samples_1 + num_samples_2, max_len, 50), dtype=np.int) * pad_token
    else:
